 ...
 * 
 * \brief SimpleHft简单高频交易策略示例类实现文件
 * 
 * 文件设计逻辑与作用总结：
 * 本文件实现了WonderTrader框架中SimpleHft简单高频交易策略的所有功能。
 * 主要功能包括：
 * 1. 实现策略的初始化和参数配置加载
...
#include "../Includes/IHftStraCtx.h"  // 包含HFT策略上下文接口，提供数据访问和交易执行接口
...
extern const char* FACT_NAME;  // 外部声明工厂名称常量，定义在WtHftStraFact.cpp中

/**
 * @brief 构造函数实现
 * @param id 策略唯一标识符，用于在系统中唯一标识该策略实例
 * 
 * 初始化SimpleHft策略对象，调用基类构造函数设置策略ID，并初始化所有成员变量。
 * 成员变量初始化说明：
 ...
 * - _stock: 初始化为false，默认为期货模式
 * - _unit: 初始化为1，默认为期货交易单位（1手）
 * - _cancel_cnt: 初始化为0，撤销订单计数器为0
 * - _reserved: 初始化为0，基础持仓量为0
 */
WtHftStraDemo::WtHftStraDemo(const char* id)  // 构造函数实现
	: HftStrategy(id)  // 调用基类构造函数，传入策略ID
	...
	, _stock(false)  // 初始化股票标志为false，默认为期货模式
	, _unit(1)  // 初始化交易单位为1，默认为期货交易单位（1手）
	, _cancel_cnt(0)  // 初始化撤销订单计数器为0
	, _reserved(0)  // 初始化基础持仓量为0
{
 ...
 */
WtHftStraDemo::~WtHftStraDemo()  // 析构函数实现
{
 ...
 */
const char* WtHftStraDemo::getName()  // 获取策略名称函数实现
{
 ...
 */
const char* WtHftStraDemo::getFactName()  // 获取工厂名称函数实现
{
...
bool WtHftStraDemo::init(WTSVariant* cfg)  // 策略初始化函数实现
{
	...
	_reserved = cfg->getDouble("reserve");  // 从配置中读取基础持仓量，转换为双精度浮点数（如果不存在则默认为0）

	_stock = cfg->getBoolean("stock");  // 从配置中读取是否为股票标志，转换为布尔值（如果不存在则默认为false）
	_unit = _stock ? 100 : 1;  // 根据股票标志设置交易单位：股票为100股，期货为1手
 ...
 */
void WtHftStraDemo::on_entrust(uint32_t localid, bool bSuccess, const char* message, const char* userTag)  // 委托回报回调函数实现
{
	// 委托回报处理为空实现，可以根据需要扩展
}
 ...
 */
void WtHftStraDemo::on_init(IHftStraCtx* ctx)  // 初始化完成回调函数实现
{
	//WTSTickSlice* ticks = ctx->stra_get_ticks(_code.c_str(), _count);  // 注释掉的代码：获取历史Tick数据（当前未使用）
	//if (ticks)  // 注释掉的代码：检查Tick数据是否获取成功
	//	ticks->release();  // 注释掉的代码：释放Tick数据切片
 ...
 * 算法步骤：
 * 1. 计算当前时间戳（微秒）
 * 2. 检查距离上次交易的时间是否小于频率限制，如果是则直接返回
 * 3. 获取最新的Tick数据
 * 4. 更新计算时间（按分钟）
 * 5. 计算理论价格：理论价格 = (买一价*卖一量 + 卖一价*买一量) / (买一量 + 卖一量)
 * 6. 比较理论价格与最新价，生成交易信号
 * 7. 根据信号和当前持仓情况执行交易操作
 * 
 * @note 该函数是私有函数，仅在策略内部调用
 */
void WtHftStraDemo::do_calc(IHftStraCtx* ctx)  // 执行策略计算逻辑函数实现
{
	const char* code = _code.c_str();  // 获取合约代码的C字符串指针，用于后续使用

	//30秒内不重复计算
	uint64_t now = TimeUtils::makeTime(ctx->stra_get_date(), ctx->stra_get_time() * 100000 + ctx->stra_get_secs());//(uint64_t)ctx->stra_get_date()*1000000000 + (uint64_t)ctx->stra_get_time()*100000 + ctx->stra_get_secs();  // 计算当前时间戳（微秒），通过日期、时间和秒数组合生成
	if (now - _last_entry_time <= _freq * 1000)  // 检查距离上次交易的时间是否小于频率限制（毫秒转微秒：_freq * 1000）
	{
		return;  // 如果时间间隔太短，直接返回，避免过度交易
	}

	WTSTickData* curTick = ctx->stra_get_last_tick(code);  // 获取最新的Tick数据，包含最新的价格和买卖盘口信息
	if (curTick == NULL)  // 检查Tick数据是否获取成功
		return;  // 如果获取失败，直接返回，不执行后续逻辑

	uint32_t curMin = curTick->actiontime() / 100000;	//actiontime是带毫秒的,要取得分钟,则需要除以10w  // 计算当前分钟数，actiontime是带毫秒的时间戳，除以100000得到分钟数
	...
	double price = curTick->price();  // 获取最新价，作为当前价格
	//计算部分
	double pxInThry = (curTick->bidprice(0)*curTick->askqty(0) + curTick->askprice(0)*curTick->bidqty(0)) / (curTick->bidqty(0) + curTick->askqty(0));  // 计算理论价格：理论价格 = (买一价*卖一量 + 卖一价*买一量) / (买一量 + 卖一量)，这是基于买卖盘口的加权平均价格
	...
	if (signal != 0)  // 检查是否有交易信号（信号不为0）
	{
		...
		curPos -= _reserved;  // 减去基础持仓量，得到实际持仓量（用于持仓修正）

		WTSCommodityInfo* cInfo = ctx->stra_get_comminfo(code);  // 获取合约信息，用于获取价格最小变动单位

		if (signal > 0 && curPos <= 0)  // 检查是否为正向信号且当前持仓小于等于0（正向信号,且当前仓位小于等于0）
		{//正向信号,且当前仓位小于等于0
			//最新价+2跳下单
			double targetPx = price + cInfo->getPriceTick() * _offset;  // 计算目标价格：最新价 + 价格最小变动单位 * 偏移跳数（最新价+2跳下单）
			auto ids = ctx->stra_buy(code, targetPx, _unit, "enterlong");  // 买入下单，返回订单ID列表

			_mtx_ords.lock();  // 锁定订单集合互斥量，保护订单集合的访问
			for (auto localid : ids)  // 遍历订单ID列表
			{
				_orders.insert(localid);  // 将订单ID插入订单集合，用于跟踪订单状态
			}
			_mtx_ords.unlock();  // 解锁订单集合互斥量
			_last_entry_time = now;  // 更新最后交易时间为当前时间，用于计算交易频率限制
		}
		else if (signal < 0 && (curPos > 0 || ((!_stock || !decimal::eq(_reserved, 0)) && curPos == 0)))  // 检查是否为反向信号且满足交易条件（反向信号,且当前仓位大于0,或者仓位为0但不是股票,或者仓位为0但是基础仓位有修正）
		{//反向信号,且当前仓位大于0,或者仓位为0但不是股票,或者仓位为0但是基础仓位有修正
			//最新价-2跳下单
			double targetPx = price - cInfo->getPriceTick()*_offset;  // 计算目标价格：最新价 - 价格最小变动单位 * 偏移跳数（最新价-2跳下单）
			auto ids = ctx->stra_sell(code, targetPx, _unit, "entershort");  // 卖出下单，返回订单ID列表
	...
	curTick->release();  // 释放Tick数据对象，调用release方法释放资源（重要：必须释放，否则会导致内存泄漏）
}
 ...
 * @note 该函数重写了HftStrategy基类的虚函数
 */
void WtHftStraDemo::on_tick(IHftStraCtx* ctx, const char* code, WTSTickData* newTick)  // Tick数据处理回调函数实现
{	
	...
	do_calc(ctx);  // 执行策略计算逻辑，生成交易信号并执行交易
}
 ...
 * @note 该函数是私有函数，仅在策略内部调用
 */
void WtHftStraDemo::check_orders()  // 检查订单状态函数实现
{
 ...
 * @note 该函数重写了HftStrategy基类的虚函数
 */
void WtHftStraDemo::on_bar(IHftStraCtx* ctx, const char* code, const char* period, uint32_t times, WTSBarStruct* newBar)  // K线闭合回调函数实现
{
	// K线数据处理为空实现，SimpleHft策略主要基于Tick数据
}
 ...
 * @note 该函数重写了HftStrategy基类的虚函数
 */
void WtHftStraDemo::on_trade(IHftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isBuy, double qty, double price, const char* userTag)  // 成交回报回调函数实现
{
 ...
 * @note 该函数重写了HftStrategy基类的虚函数
 */
void WtHftStraDemo::on_position(IHftStraCtx* ctx, const char* stdCode, bool isLong, double prevol, double preavail, double newvol, double newavail)  // 持仓回报回调函数实现
{
	// 持仓回报处理为空实现，可以根据需要扩展
}
 ...
 * 3. 更新撤销订单计数器
 * 4. 触发策略重新计算，根据新的订单状态生成交易信号
 * 
 * @note 该函数重写了HftStrategy基类的虚函数
 */
void WtHftStraDemo::on_order(IHftStraCtx* ctx, uint32_t localid, const char* stdCode, bool isBuy, double totalQty, double leftQty, double price, bool isCanceled, const char* userTag)  // 订单回报回调函数实现
{
	//如果不是我发出去的订单,我就不管了
	...
	if(isCanceled || leftQty == 0)  // 检查订单是否已撤销或全部成交（剩余数量为0）
	{
		...
		do_calc(ctx);  // 执行策略计算逻辑，根据新的订单状态生成交易信号
	}
}
 ...
 * @note 该函数重写了HftStrategy基类的虚函数
 */
void WtHftStraDemo::on_channel_ready(IHftStraCtx* ctx)  // 交易通道就绪回调函数实现
{
	...
	if (!decimal::eq(undone, 0) && _orders.empty())  // 检查是否有未完成订单且策略管理的订单集合为空（这说明有未完成单不在监控之中,先撤掉）
	{
		...
		bool isBuy = (undone > 0);  // 判断未完成订单的方向：如果未完成数量大于0，则为买入方向，否则为卖出方向
		OrderIDs ids = _ctx->stra_cancel(_code.c_str(), isBuy, undone);  // 撤销未完成订单，返回撤销的订单ID列表
 ...
 * @note 该函数重写了HftStrategy基类的虚函数
 */
void WtHftStraDemo::on_channel_lost(IHftStraCtx* ctx)  // 交易通道丢失回调函数实现
{
