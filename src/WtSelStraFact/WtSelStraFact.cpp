 ...
 * 
 * \brief SEL选股策略工厂类实现文件
 * 
 * 文件设计逻辑与作用总结：
 * 本文件实现了WonderTrader框架中SEL选股策略工厂类的所有功能。
 * 主要功能包括：
 ...
 * 
 * 该文件是SEL策略系统的核心实现，通过工厂模式实现了策略的标准化管理。
 * 支持动态库加载，可以作为插件被WonderTrader框架动态加载和卸载。
 */
#include "WtSelStraFact.h"  // 包含策略工厂类头文件
#include "WtStraDtSel.h"  // 包含DualThrust选股策略类头文件

#include <string.h>  // 包含字符串处理函数（如strcmp）
#include <boost/config.hpp>  // 包含Boost配置头文件，用于EXPORT_FLAG宏定义

const char* FACT_NAME = "WtSelStraFact";  // 定义工厂名称常量，用于标识本策略工厂
...
extern "C"  // 使用C语言链接规范，确保函数名不被C++编译器修饰
{
	/**
	 * @brief 创建策略工厂实例
	 * @return ISelStrategyFact* 返回创建的策略工厂对象指针
	 * 
	 * 该函数创建一个新的策略工厂实例并返回其指针。
	 * 该函数会被WonderTrader框架在加载动态库时调用，用于获取策略工厂实例。
	 * 
	 * @note 使用EXPORT_FLAG宏标记为导出函数，可以被外部动态调用
	 * @warning 返回的指针由调用者负责管理，使用完毕后应调用deleteSelStrategyFact删除
	 */
	EXPORT_FLAG ISelStrategyFact* createSelStrategyFact()  // 导出函数：创建策略工厂
	{
		ISelStrategyFact* fact = new WtSelStraFact();  // 创建策略工厂对象实例
	 ...
	 */
	EXPORT_FLAG void deleteSelStrategyFact(ISelStrategyFact* fact)  // 导出函数：删除策略工厂
	{
 ...
 * 如果将来需要初始化操作，可以在此处添加。
 */
WtSelStraFact::WtSelStraFact()  // 构造函数实现
{
	// 构造函数体为空，使用默认初始化
}
 ...
 * 如果将来需要清理操作，可以在此处添加。
 */
WtSelStraFact::~WtSelStraFact()  // 析构函数实现
{
	// 析构函数体为空，使用默认清理
}
 ...
 */
SelStrategy* WtSelStraFact::createStrategy(const char* name, const char* id)  // 创建策略函数实现
{
 ...
 */
void WtSelStraFact::enumStrategy(FuncEnumSelStrategyCallback cb)  // 枚举策略函数实现
{
	cb(FACT_NAME, "DualThrustSelection", true);  // 调用回调函数，传入工厂名称、策略名称和是否为最后一个策略
}
 ...
 */
const char* WtSelStraFact::getName()  // 获取工厂名称函数实现
{
