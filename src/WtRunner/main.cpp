 ...
 * \date 2020/03/30
 *
 * \brief WonderTrader策略运行器主程序入口文件
 * 
 * 文件设计逻辑与作用总结：
 * 本文件是WonderTrader策略运行器的主程序入口，提供命令行参数解析和程序启动功能。
 * 该程序是WonderTrader系统的可执行文件入口点，负责解析命令行参数、初始化运行器、加载配置并启动运行。
 * 
 * 主要功能：
 * 1. 命令行参数解析：解析配置文件路径、日志配置文件路径等命令行参数
 * 2. 崩溃转储：在Windows平台启用MiniDump崩溃转储功能，便于问题排查
 * 3. 程序启动：创建WtRunner实例，初始化日志系统，加载配置并启动运行
...
#include "WtRunner.h"  // 包含策略运行器头文件，使用WtRunner类
...
{
#ifdef _MSC_VER  // 如果是Microsoft Visual C++编译器（Windows平台）
	CMiniDumper::Enable("WtRunner.exe", true);  // 启用MiniDump崩溃转储功能
	// 参数说明：
	// - "WtRunner.exe": 模块名称，用于转储文件命名
	// - true: 是否启用完整转储
#endif
	...
	WtRunner runner;  // 创建策略运行器实例
	runner.init(filename);  // 初始化策略运行器，传入日志配置文件路径
	...
	runner.config(filename);  // 配置策略运行器，传入配置文件路径
