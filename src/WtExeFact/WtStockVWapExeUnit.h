 ...
 * \date 2020/03/30
 * 
 * \brief WtStockVWapExeUnit股票成交量加权平均价格执行单元类定义文件
 * 
 * 文件设计逻辑与作用：
 * ====================
 * 本文件定义了WtStockVWapExeUnit类，这是股票成交量加权平均价格（VWAP，Volume-Weighted Average Price）
 * 执行单元的实现类，继承自ExecuteUnit基类。VWAP执行单元根据历史成交量分布，将订单按比例分配到各个时间段执行，
 * 以实现接近成交量加权平均价格的效果，专门用于股票和可转债市场。
 * 
 * 设计目标：
 * 1. 实现股票市场的VWAP算法，根据历史成交量分布将订单按比例分配到各个时间段执行
 * 2. 支持设置执行时间段（开始时间和结束时间）
 * 3. 支持设置总执行次数，将订单分成多批执行
 * 4. 支持尾部时间处理，在最后一段时间内集中执行剩余订单
 * 5. 支持订单超时自动撤单机制
 * 6. 支持多种目标模式：股数模式、金额模式、比例模式
...
/**
 * @brief WtStockVWapExeUnit股票成交量加权平均价格执行单元类
 * 
 * 继承自ExecuteUnit基类，实现股票市场的VWAP（Volume-Weighted Average Price）算法，
...
private:
	...
	double			_min_hands{ 0 };  // 最小手数（根据股票类型自动计算：普通股票100股，科创板200股，可转债10张）
