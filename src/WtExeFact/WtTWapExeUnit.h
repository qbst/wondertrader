 ...
 * \date 2020/03/30
 * 
 * \brief WtTWapExeUnit时间加权平均价格执行单元类定义文件
 * 
 * 文件设计逻辑与作用：
 * ====================
 * 本文件定义了WtTWapExeUnit类，这是时间加权平均价格（TWAP，Time-Weighted Average Price）执行单元的实现类，
 * 继承自ExecuteUnit基类。TWAP执行单元将订单在指定时间段内均匀分配执行，以实现接近时间加权平均价格的效果。
 * 
 * 设计目标：
 * 1. 实现TWAP算法，将订单在指定时间段内均匀分配执行
...
/**
 * @brief WtTWapExeUnit时间加权平均价格执行单元类
 * 
 * 继承自ExecuteUnit基类，实现TWAP（Time-Weighted Average Price）算法，
 * 将订单在指定时间段内均匀分配执行，以实现接近时间加权平均价格的效果。
 * 
 * TWAP算法的优势：
 * - 减少市场冲击：通过分散执行，避免大单对市场造成冲击
 * - 接近平均价格：执行价格接近时间段内的平均价格
 * - 可预测性：执行计划明确，便于监控和管理
 */
