 ...
 * \date 2023/06/02
 * 
 * \brief WtStockVWapExeUnit股票成交量加权平均价格执行单元类实现文件
 * 
 * 文件设计逻辑与作用：
 * ====================
 * 本文件实现了WtStockVWapExeUnit类的所有成员函数，该类用于实现股票市场的VWAP算法，
 ...
 * By zhaoyk @ 2023.6.2
 */
#include "WtStockVWapExeUnit.h"  // WtStockVWapExeUnit类定义
...
/**
 * @brief WtStockVWapExeUnit构造函数实现
 * 
 * 创建股票VWAP执行单元实例，初始化所有成员变量为默认值。
 */
...
/**
 * @brief WtStockVWapExeUnit析构函数实现
 * 
...
{
	return "WtStockVWapExeUnit";  // 返回执行单元名称
}
 ...
 *   - offset：挂单价格偏移（相对于基准价格的偏移，买入+偏移，卖出-偏移）
 *   - lots：单次发单手数（每次下单的数量）
 *   - minopenlots：最小开仓数量（可选，默认根据品种自动确定）
 */
void WtStockVWapExeUnit::init(ExecuteContext * ctx, const char * stdCode, WTSVariant * cfg)
{
	...
	_price_offset = cfg->getUInt32("offset");  // 挂单价格偏移（相对于基准价格的偏移，买入+偏移，卖出-偏移）
	...
	ctx->writeLog(fmt::format("执行单元WtStockVWapExeUnit[{}] 初始化完成,订单超时 {} 秒,执行时限 {} 秒,收尾时间 {} 秒", stdCode, _ord_sticky, _total_secs, _tail_secs).c_str());  // 记录初始化日志
	...
	if (_min_open_lots != 0) {  // 如果用户设置了最小开仓数量
		if (_is_KC) {  // 如果是科创板股票
			_min_open_lots = max(_min_open_lots, _min_hands);  // 最小开仓数量 = max(用户设置值, 品种最小下单数)，确保不小于200股
		}
		else {  // 如果不是科创板股票
			_min_open_lots = min(_min_open_lots, _min_hands);  // 最小开仓数量 = min(用户设置值, 品种最小下单数)，确保不超过品种限制
		}
	}
 ...
 * 2. 获取未完成订单数量、目标仓位、当前仓位、可用仓位
 * 3. 检查交易通道是否就绪、是否已完成
 * 4. 根据T+1规则调整目标仓位（卖出时不能超过可用仓位）
 * 5. 检查是否有在途撤单
 * 6. 检查是否已达到目标仓位（考虑碎股情况）
 * 7. 检查是否需要撤单（未完成订单与目标方向相反）
 * 8. 检查是否有未完成订单（逐笔发单模式，有未完成订单则暂不发单）
 * 9. 检查是否有最新行情数据
 * 10. 检查是否已达到目标仓位（清仓逻辑）
 * 11. 检查是否有新行情（防止重复下单）
 * 12. 根据VWAP算法计算当前应该执行的累计数量
 * 13. 计算本次下单数量（根据VWAP预测和剩余执行次数，考虑资金限制和碎股）
...
void WtStockVWapExeUnit::do_calc()
{
	...
	double vailyPos = _ctx->getPosition(code, false);  // 可用仓位，即昨仓（T+1规则下只能卖出昨仓）
	...
	if (_is_t0)  // 如果是T+0交易模式（可转债等）
		vailyPos = realPos;  // 可用仓位 = 总仓位（T+0规则下可以卖出今仓）
	// 根据T+1规则调整目标仓位（卖出时不能超过可用仓位）
	double target_pos = max(realPos - vailyPos, _target_pos);  // 目标仓位 = max(今仓买入的, 用户设置的目标仓位)，确保卖出时不超过可用仓位
	if (!decimal::eq(target_pos, _target_pos))  // 如果调整后的目标仓位与用户设置的目标仓位不一致
	{
		_ctx->writeLog(fmtutil::format("{} can sell hold pos not enough, target adjust {}->{}", code, _target_pos, target_pos));  // 记录日志：可用仓位不足，调整目标仓位
	...
	if (decimal::eq(diffQty, 0))  // 如果目标差量为0
		return;  // 直接返回，不需要执行
	if (decimal::ge(_start_price, 0)) {  // 补一次（如果开始价格未设置，则设置为当前价格）
		_start_price = _last_tick->price();  // 设置开始价格为当前价格
	}
	// 在判断的时候，要两边四舍五入，防止一些碎股导致一直无法完成执行
	if (decimal::eq(round_hands(target_pos, _min_hands), round_hands(realPos, _min_hands)) && !(target_pos == 0 && realPos < _min_hands && realPos > target_pos))  // 如果目标仓位和当前仓位四舍五入后相等（考虑碎股）
	{
	...
	// 有未完成订单 && 与实际仓位变动方向相反
	// 则需要撤销现有订单
	bool isBuy = decimal::gt(undone, 0);  // undone>0,isbuy=1（根据未完成订单的正负判断方向）
	...
	_ctx->writeLog(fmt::format("第 {} 次发单", _fired_times + 1).c_str());  // 记录日志：第几次发单（从1开始计数）
	...
	if (isBuy) {  // 如果是买入
		// 如果是买的话，要考虑取整和资金余额
		curQty = round_hands(curQty, _min_open_lots);  // 按最小开仓数量取整
	}
	// 卖要对碎股做检查
	else {  // 如果是卖出
		if (decimal::lt(vailyPos, _min_open_lots))  // 如果可用仓位小于最小开仓数量
		{
			curQty = vailyPos;  // 下单数量 = 可用仓位（全部卖出，包括碎股）
		}
		else  // 如果可用仓位 >= 最小开仓数量
		{
			curQty = round_hands(curQty, _min_open_lots);  // 按最小开仓数量取整
		}
		curQty = min(vailyPos, curQty);  // 下单数量不能超过可用仓位（T+1规则限制，注意：这里应该是min(vailyPos, curQty)，而不是min(vailyPos, _min_open_lots)）
	}
	...
	uint64_t now = TimeUtils::getLocalTimeNow();  // 获取当前时间（毫秒时间戳）
	isBuy = decimal::gt(diffQty, 0);  // 判断买卖方向（目标差量 > 0，则为买入）
	...
	if (bNeedShowHand) {  // 如果需要亮牌（最后一批订单）
		targetPx += _comm_info->getPriceTick() * 5 * (isBuy ? 1 : -1);  // 使用更激进的价格（买入+5跳，卖出-5跳）
	}
	else if (_price_offset != 0)  // 如果价格偏移不为0
	{
		targetPx += _comm_info->getPriceTick() * _price_offset * (isBuy ? 1 : -1);  // 根据价格偏移调整价格（买入+偏移，卖出-偏移）
	}

	// 如果最后价格为0，再做一个修正
	if (decimal::eq(targetPx, 0.0))  // 如果目标价格为0
		targetPx = decimal::eq(_last_tick->price(), 0.0) ? _last_tick->preclose() : _last_tick->price();  // 如果最新价也为0，则用昨收价，否则用最新价
 ...
 * @brief 设置新的目标仓位实现
 * 
 * 设置执行单元的目标仓位（股数模式），执行单元会根据VWAP算法在指定时间段内按成交量分布执行。
 * 
 * @param stdCode 合约代码
