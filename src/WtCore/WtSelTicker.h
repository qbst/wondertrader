 ...
 * 
 * 文件设计逻辑与作用总结：
 * 本文件定义了WonderTrader框架中的选股实时行情时钟类，用于管理实时行情时间和触发分钟线闭合。
 * 主要功能包括：
 ...
 * 6. 非交易时间处理：在非交易时间，如果分钟发生变化，也会触发分钟线闭合（用于定时任务）
 * 
 * 设计模式：
 * - 观察者模式：监听行情数据，触发时间事件
 ...
 * 
 * 使用场景：
 * 该时钟主要用于选股策略场景，需要精确管理时间进度，确保分钟线能够及时闭合，
 * 同时处理行情延迟和交易日切换等特殊情况。与非交易时间的处理支持定时任务的执行。
 */
...
public:
	 ...
	 */
	WtSelRtTicker(WtSelEngine* engine);  // 构造函数
...
public:
	 ...
	 */
	void	on_tick(WTSTickData* curTick, uint32_t hotFlag = 0);  // 行情数据回调
...
private:
	 ...
	 */
	void	trigger_price(WTSTickData* curTick, uint32_t hotFlag = 0);  // 触发行情价格
