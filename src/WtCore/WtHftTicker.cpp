...
void WtHftRtTicker::on_tick(WTSTickData* curTick)
{
	...
	{
		trigger_price(curTick);  // 直接触发行情价格
		return;  // 直接返回，不进行时间管理
	}
	...
	if (_date != 0 && (uDate < _date || (uDate == _date && uTime < _time)))  // 如果已初始化且行情时间小于当前时间（时间回退）
	{
		...
		trigger_price(curTick);  // 直接触发行情价格，不更新时间
		return;  // 直接返回
	}
...
void WtHftRtTicker::run()
{
	...
	//先检查当前时间, 如果大于
	uint32_t offTime = _s_info->offsetTime(_engine->get_min_time(), true);  // 计算当前时间的偏移量（相对于开盘）

	_thrd.reset(new StdThread([this, offTime](){  // 创建监控线程
		while (!_stopped)  // 循环直到停止标志为true
		{
			...
			else //if(offTime >= _s_info->getOpenTime(true) && offTime <= _s_info->getCloseTime(true))  // 如果不在交易时间内（收盘后）
			{
				//收盘以后，如果发现上次触发的位置不等于总的分钟数，说明少了最后一分钟的闭合逻辑
				uint32_t total_mins = _s_info->getTradingMins();  // 获取交易日总分钟数
				if (_time != UINT_MAX && _last_emit_pos != 0 && _last_emit_pos < total_mins && offTime >= _s_info->getCloseTime(true))  // 如果时间已初始化、已触发过、未触发完所有分钟且已收盘
				{
					WTSLogger::warn("Tradingday {} will be ended forcely, last_emit_pos: {}, time: {}", _engine->getTradingDate(), _last_emit_pos.fetch_add(0), _time);  // 记录警告日志：强制结束交易日

					//触发数据回放模块
					StdUniqueLock lock(_mtx);  // 获取互斥锁，保护闭合事件触发

					//优先修改时间标记
					_last_emit_pos = total_mins;  // 将最后触发位置设为总分钟数（标记为已全部触发）

					bool bEndingTDate = true;  // 设置交易日结束标志（未使用）
					uint32_t thisMin = _s_info->getCloseTime(false);  // 获取收盘时间（原始时间格式）
					uint32_t offMin = _s_info->getCloseTime(true);  // 获取收盘时间（偏移分钟数）

					WTSLogger::info("Minute bar {}.{:04d} closed automatically", _date, thisMin);  // 记录分钟线闭合日志（强制闭合）
					if (_store)  // 如果数据读取器有效
						_store->onMinuteEnd(_date, thisMin, _engine->getTradingDate());  // 触发数据回放模块的分钟结束事件（传入交易日期）
				...
				{
					std::this_thread::sleep_for(std::chrono::seconds(10));  // 休眠10秒（收盘后降低检查频率）
				}
			}
