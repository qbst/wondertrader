 ...
 * \date 2020/03/30
 * 
 * \brief UFT延迟测试工具类的实现文件
 * 
 * 本文件实现了UftLatencyTool类的所有方法，以及测试用的辅助类。
 * 
 * 实现内容：
 * - UftLatencyTool类的初始化和运行逻辑
 ...
 * - 计算平均延迟（纳秒级）
 */
#include "UftLatencyTool.h"              // 包含类定义头文件
#include "../WtUftCore/UftStraContext.h"  // UFT策略上下文类
...
void test_uft()
{
	uft::UftLatencyTool runner;          // 创建UFT延迟测试工具实例
...
namespace uft                               // uft命名空间
{
	...
	public:
		...
		void	run(uint32_t times)
		{
			...
			WTSLogger::warn("{} ticks simulated in {:.0f} ns, UftEngine Innner Latency: {:.3f} ns", times, total*1.0, t2t);  // 输出测试结果：Tick数量、总耗时、平均延迟
		}
	 ...
	 * 不执行实际交易，只提供接口实现，用于测试UFT引擎的交易流程。
	 */
	...
	public:
		...
		{
			//WTSLogger::debug("{}", __FUNCTION__);  // 注释掉的调试日志
	 ...
	 * @brief 测试策略类，模拟UFT策略
	 * 
	 * 该类继承自UftStrategy，实现一个简单的测试策略。
	 * 策略逻辑：接收到Tick数据后，发出做多信号。
	 * 用于测试UFT引擎的策略执行流程。
	 */
	...
	public:
		 ...
		 * 
		 * 策略初始化时调用，订阅Tick数据。
		 * 订阅"SHFE.rb2205"合约的Tick数据。
		 */
		virtual void on_init(IUftStraCtx* ctx) override
		{
			ctx->stra_sub_ticks("SHFE.rb2205");  // 订阅螺纹钢2205合约的Tick数据
		}
		 ...
		 * 
		 * 当接收到订阅的Tick数据时调用。
		 * 策略逻辑：发出做多信号（价格2300，数量1手，滑点0）。
		 */
		virtual void on_tick(IUftStraCtx* ctx, const char* code, WTSTickData* newTick)
		{
			//WTSLogger::debug("{}", __FUNCTION__);  // 注释掉的调试日志
			ctx->stra_enter_long("SHFE.rb2205", 2300, 1, 0);  // 发出做多信号：合约、价格、数量、滑点
			//ctx->stra_enter_short("SHFE.rb2205", 2300, 1, 0);  // 注释掉的做空信号
		}
	};


	/**
	 * @brief UftLatencyTool构造函数实现
	 * 
	...
	bool UftLatencyTool::init()
	{
		...
		std::string cfgFile = "config.yaml";  // 配置文件路径

		WTSVariant* _config = WTSCfgLoader::load_from_file(cfgFile);  // 从文件加载配置文件
		...
		initEngine(_config->get("env"));  // 初始化UFT引擎，传入环境配置
	...
	{
		UftStraContext* ctx = new UftStraContext(&_engine, "stra");  // 创建策略上下文，传入引擎指针、策略ID
		...
		_engine.addContext(UftContextPtr(ctx));  // 将策略上下文添加到引擎
	...
	{
		WTSLogger::warn("Trading enviroment initialzied with engine: UFT");  // 记录日志：交易环境已初始化，使用UFT引擎
		_engine.init(cfg, &_bd_mgr, NULL, NULL);  // 初始化引擎，传入配置、基础数据管理器、数据管理器（NULL）、风险监控器（NULL）
	...
	{
		{
			...
			adapter->initExt("parser", theParser, &_engine, &_bd_mgr);  // 初始化适配器，传入ID、解析器接口、引擎、基础数据管理器
...

		{
			...
			adapter->initExt("trader", tester, &_bd_mgr, NULL);  // 初始化适配器，传入ID、交易接口、基础数据管理器、动作策略管理器（NULL）
